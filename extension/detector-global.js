...
var FILTER_MAP = Object.fromEntries(FILTERS.map((f) => [f.id, f]));
...
function detectConcerns({ description }) {
...
function fromZillowProperty(prop) {
...
function evaluateAll(listing, enabledIds) {
  const out = {};
...
;(function (root) {
  const api = { FILTERS, FILTER_MAP, detectConcerns, fromZillowProperty, evaluateAll };
  root.ZPF = Object.assign(root.ZPF || {}, api);
